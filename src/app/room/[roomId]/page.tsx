...
const Page = () => {
  ...
  const inputRef = useRef<HTMLInputElement>(null); // put focus to text element

  useRealtime({
    channels: [roomId],
    events: ["chat.messages", "chat.destroy"],
    onData: ({ event }) => {
  ...
  return (
    <main className="flex flex-col h-screen max-h-screen overflow-hidden">
      <header className="border-b border-zinc-800 p-4 flex items-center justify-content-between bg-zinc-900/30">
        ...
        {messages?.messages.length == 0 && (
          <div className="flex items-center justify-center h-full">
            <p className="text-zinc-600 text-sm font-mono">
              No messages, start sending messages.
            </p>
          </div>
        )}
